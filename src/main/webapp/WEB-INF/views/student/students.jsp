...
<div class="card">
    ...
    <div class="filter-bar">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-sm-4">
                <select id="studentRoleFilter" class="form-select form-select-sm" onchange="filterStudents()">
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <c:forEach var="s" items="${students}">
                    <tr data-name="${s.name}" data-email="${s.email}" data-role="${s.role}">
                        <td>
                        ...
                        <td>${s.phone}</td>
                        <td>${s.address}</td>
                        <td><span class="badge bg-info text-dark">${s.role}</span></td>
                        <td>
                            <c:choose>
                                <c:when test="${not empty s.lastLogin}">
                                    <small class="text-muted">${s.lastLogin}</small>
                                </c:when>
                                <c:otherwise>
...
function filterStudents() {
    ...
    const rl  = document.getElementById('studentRoleFilter').value.toUpperCase();
    filtered = allRows().filter(r =>
        (!s   || r.dataset.name.toLowerCase().includes(s) || r.dataset.email.toLowerCase().includes(s))
     && (!rl  || r.dataset.role.toUpperCase() === rl)
    );
...
function resetStudentFilters() {
    ...
    document.getElementById('studentRoleFilter').value = '';
